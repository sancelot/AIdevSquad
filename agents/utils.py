...
def parse_json_from_response(response_str):
    ...
    json_start = search_str.find('{')
    if json_start == -1:
        print("❌ Error: No JSON object found in the LLM response (no opening brace '{').")
        return None

    # Start scanning from the first brace to find the matching closing brace
    brace_count = 1
    json_end = -1
    ...
    if json_end == -1:
        print("❌ Error: Malformed JSON object in the LLM response (unbalanced braces).")
        return None
...
def configure_llm_and_embed(provider="ollama",model=None):
    ...
    if provider == "google":
        ...
        if not os.getenv("GOOGLE_API_KEY"):
            raise ValueError("Environnement variable GOOGLE_API_KEY is required to use Google Genai.")
...
def call_llm(llm_instance,system_message, user_prompt,logger=None,agent_name="Unknown Agent"):
    ...
    """
    if logger:
        logger.log_event_in_step("llm_call", {"agent_name": agent_name, "prompt": user_prompt})
    max_retries = 5
    for attempt in range(max_retries):
        ...
        try:
            ...
            if logger:
                logger.log_event_in_step("llm_response", {"response": response_content})
            ...
            if "RESOURCE_EXHAUSTED" in str(e) or "429" in str(e):
                ...
                error_details = getattr(e, 'details', {}).get('error', {}).get('details', [])
                ...
                if is_daily_quota_error:
                    print("\n" + "="*60)
                    print("🚫 Halted: Google API daily free tier quota has been exceeded.")
                    print(" The application cannot continue. Please upgrade to a paid plan or wait 24 hours.")
                    print("="*60)
                    raise e # Re-raise the exception to stop the program immediately

                # Fallback if no specific retry_delay was found
                if retry_delay <= 0:
                    retry_delay = 5 * (2 ** attempt) # Exponential backoff: 5s, 10s, 20s...
                    print(f"⚠️ Rate limit hit. Could not parse a specific delay. Retrying with generic backoff in {retry_delay} seconds...")
                else:
                    print(f"API requested a delay. Waiting for {retry_delay} seconds before retrying...")
